...
- 交通仿真：复用当前 OD 和算法，可配置车辆数、线性发车分布、时域和采样间隔；Go 端点同步触发 C++ 中观内核，返回统计、轨迹 GeoJSON 和回放数据。
...
1. 增加路口出口容量、信号灯和动态拥堵触发重规划。
2. 将仿真改为可暂停、单步、取消的异步 run，并接入分块回放和实时二进制帧。
3. 增加问题筛选、批量确认、修复操作及版本差异对比。
4. 将任务状态和地图元数据迁移到 PostgreSQL，并接入持久化工作队列。
